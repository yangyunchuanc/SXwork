...
public class ShoppingCartController {
    ...
    public int insert(@RequestBody ShoppingCart shoppingCart, HttpSession session){
        ...
        shoppingCart.setUrl(treeMapper.selectById(shoppingCart.getTreeId()).getUrl());
    ...
    public float ready(HttpSession session){
        ...
        for (ShoppingCart shoppingCart: arr) {//总共钱
            ...
            all += money;
            int b = treeMapper.selectById(shoppingCart.getTreeId()).getNumber()-shoppingCart.getNumber();
            if (b<0){
                return -2;//商品数量不够
            }
    ...
    public float start(HttpSession session){
        ...
        for (ShoppingCart shoppingCart: arr){
            ...
            //更新商品数量
            int b = treeMapper.selectById(shoppingCart.getTreeId()).getNumber()-shoppingCart.getNumber();
            treeMapper.updateGoodNum(b,shoppingCart.getTreeId());
            //卖家所得
            User user2 = UserMapper.selectById(treeMapper.selectById(shoppingCart.getTreeId()).getUserId());
